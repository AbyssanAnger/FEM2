...
def analysis():
    ...
    x_grid, y_grid = torch.meshgrid(x_coords, y_coords, indexing='ij') # Gitter von Knotenkoordinaten Beispiel: (0,0), (0,0.005), ..., (2,0.05)
    x = torch.stack((x_grid.flatten(), y_grid.flatten()), dim=1) # Knotenkoordinaten als Tensor [nnp x ndm] Beispiel: [[0,0], [0,0.005], ..., [2,0.05]]
    ...
    elems = torch.tensor(elements_list, dtype=torch.long) # Elemente als Tensor [nel x nen] Beispiel: [[0,11,12,1], [1,12,13,2], ...]
    ...
    free_mask = torch.ones(nnp * ndf, 1) - drlt_mask # (torch.ones füllt ALLES mit 1en) (freie Freiheitsgrade sind 1, festgehaltene 0) (Inverse von drlt_mask)
...
# #####NEWMARK-TIME INTEGRATION SCHEME STARTS HERE#####
    ...
    for s in range(steps):
        ...
        F_1 = fsur + M.mm( (1/(beta*dt*dt)*u_0) + (1/(beta*dt)*v_0) + ((1-2*beta)/(2*beta))*a_0)
